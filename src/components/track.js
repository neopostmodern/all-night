...
var Track = (props) => {
  ...
  if (meta.venue) {
    ...
    if (icon) {
      venue = (
        <img className='venue' src={icon} alt={venue_name} title={venue_name} />
      )
    } else {
  ...
  if (meta.location) {
    context_information.push(
      <span className='location' key={track.id + '-loc'}>
        {meta.location}
      </span>,
    )
  }
  ...
  let popularityOrProgress = (
    <div className='popularity' title={track.playback_count + ' plays'}>
      <div
        className='bar'
        style={{
          width: Math.max(Math.log10(track.playback_count) - 2, 0) + 'rem',
        }}
      />
    </div>
  )
  ...
  if (props.history === 1) {
    userRelation = (
      <i
        ...
        style={{ color: 'gray' }}
        title="You've heard part of this track"
      >
        done
      </i>
    )
  ...
  if (track.liked) {
    userRelation = (
      <i
        className='material-icons'
        style={{ color: 'darkred' }}
        title="You've favorited this track"
      >
        favorite
      </i>
    )
  ...
  return (
    <div className={classes}>
      ...
      {context_information}
      <div className='user-relation'>{userRelation}</div>
      <div className='artist'>
        <a
          href={track.user.permalink_url}
          target='_blank'
          title={track.user.username + "'s profile on SoundCloud"}
        >
          {track.user.username}
        </a>
      </div>
      ...
      <div className='origin'>
        <a
          ...
          target='_blank'
          title='Go to track on SoundCloud'
        >
