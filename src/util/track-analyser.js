...
let specialPodcastTreatments = {
  ...
  'Radio Juicy Vol': (title, analysis) => {
    let number = parseInt(
      title.match(/Radio Juicy Vol\. \d+/i)[0].substring(17),
    )
  ...
  'Deep Afterhour Nr': (title, analysis) => {
    let number = parseInt(
      title.match(/Deep Afterhour Nr\. \d+/i)[0].substring(19),
    )
...
const PODCAST_REGEX =
  /((\-|\|)\s*)?[^(\-|\|)]+((cast)|(mix))\s?(#\s?)?\d{1,3}(\s*(-|(by)))?/gi // todo: abstract redundant patterns
